...
export async function handleGetPluginManifest(
  ...
  const res = await client.get<PluginResponse>(
    `/api/plugins/${params.pluginName}`,
  );
  return res.ok ? jsonResult(res.data) : errorResult(res.error);
}
...
export async function handleGetConfig(
  ...
  const res = await client.get<ConfigFileResponse>(
    `/api/plugins/${params.pluginName}/configs/${params.filename}`,
  );
  return res.ok ? jsonResult(res.data) : errorResult(res.error);
}
...
export function registerPluginTools(
  ...
  server.registerTool("list_plugins", {
    description: "List registered model plugins with training phases and generation modes",
    inputSchema: {},
  ...
  server.registerTool("get_plugin_manifest", {
    ...
    inputSchema: {
      pluginName: z.string().describe("Plugin name (e.g. 'tidal')"),
    },
