...
describe("Database.createUserPlugin()", () => {
  it("creates a plugin with all fields", async () => {
    const dir = await freshTmpDir();
    const db = createDb(dir);

    const user = db.upsertUser({ githubId: 1, githubLogin: "alice", githubAvatarUrl: null });
    const plugin = db.createUserPlugin({
      userId: user.id,
      name: "my-model",
    ...
    assert.ok(plugin.id);
    assert.equal(plugin.userId, user.id);
    assert.equal(plugin.name, "my-model");
    assert.equal(plugin.displayName, "My Model");
    assert.ok(plugin.createdAt > 0);
  ...
  it("enforces unique (user_id, name)", async () => {
    ...
    const user = db.upsertUser({ githubId: 1, githubLogin: "alice", githubAvatarUrl: null });
    db.createUserPlugin({ userId: user.id, name: "dup", displayName: "First" });

    assert.throws(
      () => db.createUserPlugin({ userId: user.id, name: "dup", displayName: "Second" }),
    );
...
describe("Database.getUserPlugin()", () => {
  it("returns a plugin by id", async () => {
    const dir = await freshTmpDir();
    const db = createDb(dir);

    const user = db.upsertUser({ githubId: 1, githubLogin: "alice", githubAvatarUrl: null });
    const created = db.createUserPlugin({ userId: user.id, name: "test", displayName: "Test" });

    const found = db.getUserPlugin(created.id);
    assert.ok(found);
  ...
  it("returns null for non-existent id", async () => {
    ...
    assert.equal(db.getUserPlugin("nonexistent"), null);

    db.close();
  });
});

describe("Database.getUserPluginByUserAndName()", () => {
  it("finds a plugin by user id and name", async () => {
    const dir = await freshTmpDir();
    const db = createDb(dir);

    const user = db.upsertUser({ githubId: 1, githubLogin: "alice", githubAvatarUrl: null });
    db.createUserPlugin({ userId: user.id, name: "unique", displayName: "Unique" });

    const found = db.getUserPluginByUserAndName(user.id, "unique");
    assert.ok(found);
    assert.equal(found!.displayName, "Unique");
  ...
  it("returns null when name not found", async () => {
    ...
    const user = db.upsertUser({ githubId: 1, githubLogin: "alice", githubAvatarUrl: null });
    assert.equal(db.getUserPluginByUserAndName(user.id, "nope"), null);

    db.close();
  });
});

describe("Database.listUserPluginsByUser()", () => {
  it("returns only plugins for the given user", async () => {
    const dir = await freshTmpDir();
    const db = createDb(dir);
    ...
    db.createUserPlugin({ userId: alice.id, name: "a-plugin", displayName: "Alice Plugin" });
    db.createUserPlugin({ userId: bob.id, name: "b-plugin", displayName: "Bob Plugin" });

    const alicePlugins = db.listUserPluginsByUser(alice.id);
    assert.equal(alicePlugins.length, 1);
    assert.equal(alicePlugins[0].name, "a-plugin");

    db.close();
  });
});

describe("Database.updateUserPlugin()", () => {
  it("updates display name", async () => {
    const dir = await freshTmpDir();
    const db = createDb(dir);

    const user = db.upsertUser({ githubId: 1, githubLogin: "alice", githubAvatarUrl: null });
    const created = db.createUserPlugin({ userId: user.id, name: "test", displayName: "Old" });

    const updated = db.updateUserPlugin(created.id, { displayName: "New" });
    assert.ok(updated);
  ...
  it("returns null for non-existent id", async () => {
    ...
    assert.equal(db.updateUserPlugin("nonexistent", { displayName: "X" }), null);

    db.close();
  });
});

describe("Database.deleteUserPlugin()", () => {
  it("deletes an existing plugin", async () => {
    const dir = await freshTmpDir();
    const db = createDb(dir);

    const user = db.upsertUser({ githubId: 1, githubLogin: "alice", githubAvatarUrl: null });
    const created = db.createUserPlugin({ userId: user.id, name: "del", displayName: "Del" });

    assert.equal(db.deleteUserPlugin(created.id), true);
    assert.equal(db.getUserPlugin(created.id), null);
  ...
  it("returns false for non-existent id", async () => {
    ...
    assert.equal(db.deleteUserPlugin("nonexistent"), false);
